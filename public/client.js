// invoke socket.io
var socket = io("http://localhost.com");
// get users to join the game
...
TicTacToe.prototype = { // gives instances of TicTacToe the below methods and values
    ...
    getSquareValues: function(id) {
        ...
        // stop clicking a square with a value
        if (squareElem && squareElem.value) {
            return;
        }
        // play turn if square is free and game is not over
        if (squareElem && squareElem.value == "" && this.gameOver !== true) { // stops a square being selected more than once
            if (this.xTurn) {
                this.increment();
                squareElem.value = "X";
                this.xTurn = false;
